...
HF Inference Providers' free monthly credits ran out mid-experiment, and
this keeps the pipeline fully free and reproducible on any machine with
Ollama installed, at the cost of running on local compute instead of
dedicated inference hardware. Start the server with `ollama serve` and
pull the model with `ollama pull llama3.1:8b` before running experiments.
"""
