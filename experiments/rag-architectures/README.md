...
**Dense retrieval (semantic search) vs. BM25.** Two different, opposite ways to rank chunks against a query, and the two signals Hybrid RAG in this experiment fuses together.

- **Dense retrieval** embeds the query and every candidate chunk as vectors, then ranks chunks by how close their vector is to the query's (cosine similarity). Called "dense" because every dimension of the vector typically carries some signal, as opposed to a sparse, mostly-zero representation like a keyword count. Its strength is meaning: "who leads the company" can match a chunk that says "chief executive officer" despite sharing no words at all, because both phrases land near each other in embedding space. Its weakness is exactly that same fuzziness: it can miss an exact name, date, or number that a keyword search would catch instantly, because those specific tokens get blended into the same vector as everything else in the chunk.
- **BM25** is a classic, decades-old keyword-ranking function ([Robertson & Zaragoza](https://www.staff.city.ac.uk/~sb317/papers/foundations_bm25_review.pdf)) that scores a chunk against a query purely by term frequency: how often the query's actual words appear in the chunk, weighted by how rare those words are across the whole corpus. It has no notion of meaning at all, so it can't find a paraphrase the way dense retrieval can, but it never misses an exact name or number that's actually written in the text, since it's matching literal words rather than a compressed vector representation of them.

In short: dense retrieval finds a chunk that *means* the same thing; BM25 finds a chunk that *says* the same thing. A query that's a close paraphrase of the source text favors dense retrieval; a query built around an exact proper noun or figure favors BM25. Hybrid RAG runs both and merges the two rankings (see Reciprocal rank fusion, below) specifically because a real corpus has both kinds of queries, and picking only one leaves the other kind's failures uncaught.
...
**Exact Match / F1.** Two ways of scoring a predicted answer against the correct one. Exact Match is strict: 1 if the (normalized) strings match exactly, 0 otherwise. F1 gives partial credit based on word overlap, so a predicted answer that's mostly right but missing or adding a word still scores above zero. Both are standard for extractive QA datasets.

**Abstention (correct / incorrect).** Whether an architecture said "Insufficient information." instead of giving a real answer. Abstaining is *correct* on a null query, where the corpus genuinely has no answer, and *incorrect* on an answerable query, where it means the architecture gave up on a question it could have answered. The same behavior (refusing to answer) is a win in one case and a failure in the other, which is why this experiment tracks the two rates separately rather than as one "abstention rate."

**LLM calls / retrieval rounds.** How many separate calls to the language model, and how many separate retrieval attempts, one query needed. These are the direct, mechanical source of an architecture's extra cost: HyDE and Query Decomposition each make 2 LLM calls per query (versus 1 for the single-pass architectures) because each needs an extra call before the final answer (writing a hypothesis, or decomposing the query). A "round" counts a retry as a new attempt; a multi-query fan-out (several sub-queries searched at once) still counts as 1 round, since nothing failed to trigger a retry there.

**Prompt tokens / completion tokens.** How much text, measured in tokens (roughly, word-pieces), a call sends to the model (prompt) and gets back (completion). Prompt tokens track directly with how many chunks and how big they are; completion tokens track with how much the model had to write, which is why HyDE and Query Decomposition, whose first call generates a whole hypothesis or a list of sub-questions, show far higher completion-token counts than the single-call architectures, whose one call just needs a short "Answer: ...".

**Latency / wall-clock time.** Latency is how long a single model call took, start to finish. Wall-clock time is how long the *whole* query took, including every retrieval call and every LLM call in the run, which is what actually matters to whoever is waiting on an answer. For a single-call architecture the two numbers are close to identical; for a 2-call architecture like CRAG, wall-clock time is latency summed across both calls plus the retrieval steps in between.

**Time to first token (TTFT).** How long a model takes to start responding, as opposed to latency, which includes the time to finish the whole response. It's the metric closest to how responsive a system *feels*, since a user (or the next step in a pipeline) is waiting on the first token, not the last one. See this experiment's own caveat on TTFT under "Why these metrics" above: this run's TTFT numbers needed a median rather than a mean, because of a real local-serving artifact hit partway through.

**Context payload bytes.** The size, in bytes, of everything sent to the model in one call: the system prompt, the passages, the question. It's a proxy for how much the model has to read before it can answer, distinct from token count because it's measured before tokenization.

**Empty retrieval rate.** How often a retrieval call came back with literally zero chunks. With a shared corpus of thousands of chunks, a plain top-k search essentially never returns fewer than k results, so this metric mostly matters for an architecture (like CRAG's corrective branch, or Query Decomposition's per-sub-question searches) where a specific, narrower search could plausibly find nothing.

**Error rate.** How often a model call failed outright (a timeout, a malformed response, a dropped connection), as opposed to succeeding but giving a wrong or unhelpful answer. A 0% error rate, as every architecture had here, means every quality difference in the results is a genuine behavioral difference, not one architecture just breaking more often than another.
