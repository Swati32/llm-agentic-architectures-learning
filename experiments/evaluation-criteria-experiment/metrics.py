...
operational metrics this experiment compares system shapes on. See
CLAUDE.md for the metric definitions, and this experiment's README for
`redundant_step_rate`, the one metric this experiment adds beyond the
standard list.
"""
...
def quality_summary(records: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for shape, group in records.groupby("shape"):
        rows.append(
            {
                "shape": shape,
        ...
        rows.append(
            {
                ...
                "redundant_step_rate": group["redundant_step_rate"].mean(),
