"""Shared plumbing every system shape builds on: the step/run record shape,
and the LLM sub-tasks a shape can reuse verbatim. Keeping these identical
across shapes means any difference in the results comes from *shape*, what
...
@dataclass
class Step:
    role: str  # e.g. "agent", "supervisor", "reasoner", "verifier"
    ...
    retrieved_title: str | None = None  # only set for tool_call steps
    error: str | None = None
...
def tool_step(role: str, query: str, result: dict | None, latency_seconds: float, error: str | None) -> Step:
    return Step(
        ...
        retrieved_title=result["title"] if result else None,
        error=error,
    )
...
class RunResult:
    ...
    wall_clock_seconds: float  # actual elapsed time
    ...
    def empty_retrievals(self) -> int:
        ...
        gold paragraphs for the question — search() with k=1 never returns
    ...
    @property
    def redundant_retrievals(self) -> int:
        """A tool call that retrieved a paragraph title an earlier tool call
        in this same run already retrieved. Any later duplicate counts, not
        just an immediate repeat, since a run that re-searches the same
        title two rounds apart is still redoing work it already did."""
        seen_titles: set[str] = set()
        redundant = 0
        for step in self.steps:
            if step.kind != "tool_call" or step.retrieved_title is None:
                continue
            if step.retrieved_title in seen_titles:
                redundant += 1
            seen_titles.add(step.retrieved_title)
        return redundant

    @property
    def redundant_step_rate(self) -> float:
        return self.redundant_retrievals / self.tool_calls if self.tool_calls else 0.0
