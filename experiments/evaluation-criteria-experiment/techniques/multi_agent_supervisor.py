"""The multi-agent shape: a Supervisor coordinating three specialist roles,
Retriever, Reasoner, Verifier, with an explicit self-correction loop. A
draft answer is only accepted once a separate Verifier role checks it's
...
NAME = "Multi-Agent (Supervisor + Verification Loop)"
DESCRIPTION = (
    ...
    "bounded to a fixed number of rounds. More than one role, handing off control."
)
MAX_ROUNDS = 3
...
WHAT_IT_IS = (
    "The one shape in this experiment with more than one role. The Supervisor doesn't retrieve, "
    "draft, or check anything itself, it routes between three specialists that each do one job: "
    "a Retriever fetches evidence, a Reasoner drafts an answer from it, and a Verifier, a "
    "separate role with no stake in the draft being right, checks whether the evidence actually "
    "supports it. A rejected draft sends control back to the Supervisor to try a different "
    "search, rather than accepting the first thing the Reasoner produced. This is the "
    "reflection/self-critique idea from [Reflexion](https://arxiv.org/abs/2303.11366) and "
...
HOW_WE_IMPLEMENTED_IT = (
    "A loop bounded to 3 rounds, using the exact same `search()` tool as every other shape in "
    "this experiment. Round 1 searches the original question; the Reasoner drafts an answer and "
    "quotes the evidence sentence it's relying on; the Verifier checks that quote against the "
    "draft and responds `SUPPORTED` or `NOT_SUPPORTED[reason]`. If supported, that draft becomes "
    "the final answer immediately. If not, a Supervisor call is shown the Verifier's stated "
    "reason and writes an improved search query for the next round. Every round costs 3 handoffs "
    "(Supervisor to Retriever, Retriever to Reasoner, Reasoner to Verifier), plus one more "
    "(Verifier back to Supervisor) if it loops again. If round 3 still isn't verified, that "
    "round's draft is taken anyway and the run is flagged `early_terminated`. Whether a later "
    "round's refined query lands on the *same* paragraph an earlier round already retrieved is "
    "tracked as `redundant_step_rate`, this experiment's test of whether query refinement is "
    "actually finding new evidence or just re-finding what it already had."
)
WHEN_ITS_USEFUL = (
    "Reach for a second, specialist role, not just a bigger loop, when a wrong answer is more "
    "costly than a slow one, and specifically when the way an answer can be wrong is 'sounds "
    "plausible but isn't actually backed by the evidence', exactly the failure mode a dedicated "
    "Verifier role is positioned to catch, since it never sees the Reasoner's confidence, only "
    "the quoted evidence. It costs more rounds, more tokens, and more coordination (handoffs) "
    "than a single agent looping alone; whether that coordination is worth it, and whether it "
    "fails in the specific ways multi-agent systems are known to fail (see the README's "
    "'Multi-agent coordination failure' section), is exactly what this experiment measures."
)
